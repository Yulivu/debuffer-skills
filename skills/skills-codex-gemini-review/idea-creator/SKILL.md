...
### Phase 0: Load Reference-Led Topic Seeds (if available)

Before the landscape survey, check `idea-stage/TOPIC_CANDIDATES.md`. If it
exists, read it as a compact evidence handoff. Consult
`idea-stage/REFERENCE_DECONSTRUCTION.md` only when a locator or boundary needs
interpretation; do not redo its paper or figure evidence extraction.
...
repairable question is supported.
