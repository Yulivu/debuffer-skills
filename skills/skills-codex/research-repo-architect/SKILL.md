...
6. Run the repository's inventory and targeted tests after integration. In Codex-managed projects, resolve the skill repo root from `.debuffer_skills/installed-skills-codex.txt` or `$SKILL_REPO` first, then run the checks from that repo:

```powershell
$repo = if ($env:SKILL_REPO) { $env:SKILL_REPO } else { (Select-String -Path ".debuffer_skills/installed-skills-codex.txt" -Pattern "^repo_root`t").Line.Split("`t")[1] }
Push-Location $repo
python "$repo/tools/check_skills_inventory.py"
python -m pytest "$repo/tests/test_codex_skill_mirror.py" "$repo/tests/test_codex_install_update.py" "$repo/tests/test_install_*_tools_symlink.py"
Pop-Location
```
