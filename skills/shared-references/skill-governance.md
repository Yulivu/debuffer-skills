...
When debuffer **auto-writes** a durable artifact — a meta-optimize patch to a SKILL.md,
...
- **Auto-curation may ONLY touch artifacts where `is_auto_authored(path)` is True.**
  ...
  `created_by == "debuffer-auto"`. Canonical hand-written skills and user notes have no
...
a *captured skill* that an unreviewed loop grants itself authority to keep. debuffer's
