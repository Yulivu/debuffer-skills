...
Every debuffer compute skill (`/run-experiment`, `/experiment-queue`,
...
| Shape | debuffer examples | Build = | Env name resolves to |
|---|---|---|---|
...
Per provider, keep an append-friendly `.debuffer_skills/compute/<provider>.md` (or the
...
the env is the same as last week" into a string compdebufferon. Note `.debuffer_skills/` is
gitignored by convention — the ledger is **project-local and uncommitted** by
default (like `.debuffer_skills/traces/`). If you want committed, git-blameable history,
...
runtime onto plain bash + SSH + debuffer subagents.
