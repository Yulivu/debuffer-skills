...
When debuffer captures *durable* knowledge — a research-wiki idea / claim / experiment
...
debuffer's research-wiki "failed ideas → anti-repeat memory" is the GOOD inverse (a
...
- **Mechanical** (deterministic, `tools/capture_filter.py`): the unambiguous
  ...
  / network), and explicitly-broken-tool phrasing anchored on debuffer infrastructure
...
to OOD", "our method fails on long sequences") — it targets debuffer's own *tooling*
...
> "Do NOT capture" list (MIT). debuffer's increment: Hermes patches self-poisoning with
> more self-judged prose; debuffer adds the deterministic screen + the cross-model
