...
After each `spawn_agent` or optional `oracle-pro` reviewer call, save the trace following `../../skills-codex/shared-references/review-tracing.md`. Write files directly to `.debuffer_skills/traces/novelty-check/<date>_run<NN>/` and record searched claims, closest papers, reviewer route, raw response, and final novelty decision. Respect the `--- trace:` parameter when present (default: `full`).
