...
if research-wiki/ exists:
    ...
    # 4. Rebuild + log
    rebuild the query pack with the installed debuffer research_wiki.py helper
    log "result-to-claim: exp:<id> verdict=<verdict> for idea:<idea_id>" with the installed debuffer research_wiki.py helper
...
After the secondary Codex judgment, save a trace following `../../skills-codex/shared-references/review-tracing.md`. Write files directly to `.debuffer_skills/traces/result-to-claim/<date>_run<NN>/` and include the prompt, raw reviewer response, parsed verdict, routing action, and whether the result is `[pending external review]`. Respect the `--- trace:` parameter when present (default: `full`).
