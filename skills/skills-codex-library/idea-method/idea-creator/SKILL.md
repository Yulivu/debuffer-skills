...
## Phase 0: Load Research Wiki

When `research-wiki/` exists, load `research-wiki/query_pack.md` before
generation. Treat its gaps as search seeds and its failed ideas as a banlist.
When a candidate is accepted, use the repository helper to **Write Ideas to Research Wiki** as a durable record. Do not store unverified novelty claims as facts.
...
Read `../../skills-codex/shared-references/autosci-lite-patterns.md`. Use the
paths as a coverage menu, not a quota:
...
For each surviving candidate, add `prior_work`, `so_what`, `effort_note`, and
`minimum_validation`.
