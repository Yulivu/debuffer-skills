...
6. **Cross-model independence.** Per-page Codex calls in `/slides-polish` use fresh `spawn_agent` calls (no `send_input`). See `../../skills-codex/shared-references/reviewer-independence.md`.
7. **Anonymity fail-closed.** If any audit (or any Codex fix proposal) would replace a placeholder with a real title / count / URL, the workflow halts and surfaces the proposal for human review. See `../../skills-codex/shared-references/experiment-integrity.md`.
...
`../../skills-codex/shared-references/assurance-contract.md`.
...
See `../../skills-codex/shared-references/effort-contract.md`.

| `effort` | Behavior |
|---|---|
| `lite` | Forwarded to sub-skills. `/slides-polish` runs in `lite` (BLOCKERS only). **Phase 4 still runs at the requested assurance level** — `effort` controls depth, never audit gating (see `../../skills-codex/shared-references/assurance-contract.md`). |
