...
- **REVIEWER_MODEL = `gpt-5.5`** — Codex MCP model for per-page review. xhigh reasoning is non-negotiable (see `../../skills-codex/shared-references/effort-contract.md`). `gpt-5.4` is acceptable when the user has no `gpt-5.5` access; `gpt-5.5` is preferred for visual nuance.
- **REVIEWER_REASONING = `xhigh`** — Hard invariant; the effort knob does **not** change this.
- **CONTEXT_POLICY = `fresh`** — Each per-page review uses a **fresh** Codex reviewer call (`spawn_agent`, never `send_input`). See `../../skills-codex/shared-references/reviewer-independence.md`. This prevents the reviewer from anchoring on prior fixes.
...
.debuffer_skills/slides-polish/<deck-stem>/
...
`.debuffer_skills/slides-polish/<deck-stem>/`. Per-call Codex traces also follow the
shared convention `.debuffer_skills/traces/slides-polish/<date>_runNN/` per
`../../skills-codex/shared-references/review-tracing.md`. Resumable across sessions if
...
in `.debuffer_skills/` to keep the deck directory free of polish-specific cruft.
...
6. **Render PDF → PNG**: `pdftoppm -r 150 <pdf> .debuffer_skills/slides-polish/<stem>/png/page` produces one PNG per slide; passed to Codex during per-page review.
...
`.debuffer_skills/slides-polish/<deck-stem>/inspect_pptx.py`, create it from this
      ...
      "shapes": [
        {
          ...
          "text": "debuffer",
          "runs": [
            {"text": "debuffer", "font_pt": 80.0, "bold": false,
...
- **Anonymity placeholder discipline**: ALL placeholders for under-review
  ...
  See `../../skills-codex/shared-references/experiment-integrity.md`.
...
See `../../skills-codex/shared-references/effort-contract.md` for the full contract.
...
`../../skills-codex/shared-references/review-tracing.md`. Per-call file under
`.debuffer_skills/slides-polish/<stem>/traces/slide_KK.json` with:
