...
- A related paper is not automatically direct coverage. Classify each overlap
  as `direct`, `partial`, `incomparable`, or `insufficient-evidence`.
...
- Maintain an append-only prior-art ledger containing paper identity, source
  verification, overlapping claim, difference, status, and the reason for every
  status change. Do not remove unfavorable evidence from later drafts.
