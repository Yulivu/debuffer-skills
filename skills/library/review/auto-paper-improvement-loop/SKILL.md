...
- **Never pass `— style-ref` (or the cache contents) to the reviewer.** The
  Reviewer Independence Protocol requires reviewers see only the artifact and
  the user's prompt; leaking the style ref would contaminate the review with
  author-side context.
