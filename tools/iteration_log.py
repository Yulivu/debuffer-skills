...
The ledger is a sidecar at `.debuffer_skills/runs/<run_id>.iterations.jsonl`; it deliberately does
NOT import or touch run_state.py's done/accepted state machine (only shares the `.debuffer_skills/runs/`
...
def _log_path(root: str, run_id: str) -> Path:
    ...
    return _state_dir(root) / "runs" / f"{run_id}.iterations.jsonl"


def _state_dir(root: str) -> Path:
    configured = Path(root) / STATE_DIR_NAME
