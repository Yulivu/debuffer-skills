...
def _arxiv_user_agent() -> str:
    ...
    The contact is read from ``DEBUFFER_VERIFY_EMAIL`` (or legacy
    ``ARIS_VERIFY_EMAIL``) so no address is hard-coded. Falls back to a
    contactless UA when unset.
    """
    contact = (
        os.environ.get("DEBUFFER_VERIFY_EMAIL")
        or os.environ.get("debuffer_VERIFY_EMAIL")
        or os.environ.get("ARIS_VERIFY_EMAIL")
        or ""
    ).strip()
