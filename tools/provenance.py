#!/usr/bin/env python3
"""Provenance-as-authorization for debuffer auto-authored artifacts.

The primitive debuffer needs the MOMENT it auto-writes a skill / memory node: a
...
NousResearch/hermes-agent's skill_provenance ContextVar, MIT). debuffer's increment:
Hermes records only `created_by`, and its cross-model curator is OPTIONAL config
(defaults to the SAME chat model). debuffer records the RICHER tuple
...
# can't substring-bleed into an unrelated name. debuffer routes oracle-pro to a
...
def stamp(target: str, author_model: str, reviewer_model: str, verdict_id: str,
          created_by: str = "debuffer-auto", ts: Optional[str] = None) -> dict:
...
def is_auto_authored(target: str) -> bool:
    ...
    rec = read(target)
    return bool(rec and rec.get("created_by") == "debuffer-auto")
...
def main() -> int:
    ap = argparse.ArgumentParser(description="debuffer provenance-as-authorization.")
    ...
    try:
        if a.cmd == "stamp":
            ...
            print("debuffer-auto" if ok else "not debuffer-auto (canonical/user — off-limits to auto-curation)")
