#!/usr/bin/env python3
"""Regression test for install_aris.sh: project-local .debuffer_skills/tools symlink.

Covers:
  install: fresh project gets `.debuffer_skills/tools -> <repo>/tools`
  install: idempotent on rerun
  install: warns and leaves alone if `.debuffer_skills/tools` already exists with
           a different target / as a non-symlink path
  uninstall: removes the managed symlink
  uninstall: preserves non-managed `.debuffer_skills/tools` (different target / real dir)
...
@unittest.skipIf(os.name == "nt" and shutil.which("bash") is None, "install_aris.sh tests require bash")
class InstallTest(unittest.TestCase):
    ...
    def test_install_creates_tools_symlink(self):
        ...
        link = self.project / ".debuffer_skills" / "tools"
    ...
    def test_install_dry_run_does_not_create_symlink(self):
        ...
        self.assertIn(".debuffer_skills/tools", result.stdout, "dry-run output must mention .debuffer_skills/tools")
    ...
    def test_install_does_not_replace_existing_dir(self):
        # User already has a real .debuffer_skills/tools dir; installer must leave it alone
        (self.project / ".debuffer_skills").mkdir()
        existing_dir = self.project / ".debuffer_skills" / "tools"
        existing_dir.mkdir()
        ...
        result = self._run()
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertFalse((self.project / ".debuffer_skills" / "tools").is_symlink())
    ...
    def test_install_does_not_replace_different_symlink(self):
        # User already has .debuffer_skills/tools pointing somewhere else
        (self.project / ".debuffer_skills").mkdir()
        elsewhere = self.tmp / "elsewhere"
        elsewhere.mkdir()
        os.symlink(str(elsewhere), str(self.project / ".debuffer_skills" / "tools"))

        result = self._run()
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(
            os.readlink(self.project / ".debuffer_skills" / "tools"),
            str(elsewhere),
    ...
    def test_uninstall_removes_managed_symlink(self):
        self._run()  # install
        self.assertTrue((self.project / ".debuffer_skills" / "tools").is_symlink())
        result = self._run("--uninstall")
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertFalse(
            (self.project / ".debuffer_skills" / "tools").exists(),
